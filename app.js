...
buttonsElement.addEventListener('click', (event) => {
    ...
    if (texts[eventNumber][decision+6] !== 'death' && texts[eventNumber][decision+6] !== 'victory' && eventNumber < texts.length-2){
        nextScene();
    }
    // Final boss code to show only the options the user has enough loot for
    else if (texts[eventNumber][decision+6] !== 'death' && texts[eventNumber][decision+6] !== 'victory' && eventNumber >= texts.length-2) {
        nextScene();
        if (loot < 2) {
